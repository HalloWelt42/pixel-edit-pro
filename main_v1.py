...
class PixelCanvas(QWidget):
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton and self.drawing:
            ...
            if self.preview_pixmap and self.draw_mode != DrawMode.POLYGON:
                self.apply_preview()
                self.preview_pixmap = None
    ...
    def update_preview(self, current_pos):
        ...
        if self.draw_mode == DrawMode.LINE:
            ...
            radius = int(((current_pos.x() - center.x()) ** 2 +
                          (current_pos.y() - center.y()) ** 2) ** 0.5)
...
class PixelEditor(QMainWindow):
    ...
    def create_tools_panel(self):
        ...
        tools = [
            ...
            ("●", DrawMode.FILLED_CIRCLE, "Filled Circle"),
    ...
    def choose_primary_color(self):
        color = QColorDialog.getColor(self.canvas.primary_color, self)
        if color.isValid():
            self.set_primary_color(color)

    def choose_secondary_color(self):
        color = QColorDialog.getColor(self.canvas.secondary_color, self)
        if color.isValid():
    ...
    def update_layers_list(self):
        ...
        for i, layer in enumerate(self.canvas.layers):
            item = QListWidgetItem(f"{layer.name} {'👁' if layer.visible else '🚫'}")
    ...
    def open_file(self):
        ...
        if filename:
            ...
            if not image.isNull():
                # Scale to grid
                scaled = image.scaled(
                    self.canvas.grid_size, self.canvas.grid_size,
                    Qt.AspectRatioMode.KeepAspectRatio,
                    Qt.TransformationMode.SmoothTransformation
                )
                self.canvas.layers[0].pixmap = QPixmap.fromImage(scaled)
                self.canvas.update()
            ...
            for x in range(1, self.canvas.grid_size - 1):
                ...
                for dy in [-1, 0, 1]:
                    for dx in [-1, 0, 1]:
                        ...
                        b += pixel.blue()
                        count += 1
                color = QColor(r // count, g // count, b // count)
